...
def build_invoice_query(version):
    return f"""<?xml version="1.0" encoding="utf-8"?>
<?qbxml version="{version}"?>
<QBXML>
  <QBXMLMsgsRq onError="stopOnError">
...
def build_invoice_mod(version, txnid, edit_seq, ref_number):
    safe_ref = saxutils.escape(ref_number)
    return f"""<?xml version="1.0" encoding="utf-8"?>
<?qbxml version="{version}"?>
<QBXML>
  <QBXMLMsgsRq onError="stopOnError">
...
def fetch_invoices(rp, ticket, version):
    request = build_invoice_query(version)
    response = rp.ProcessRequest(ticket, request)
    root = ET.fromstring(response)
    return root.findall(".//InvoiceRet")

def auto_number_invoices(rp, ticket, version, log_callback):
    invoices = fetch_invoices(rp, ticket, version)
    used_numbers = [
        int(inv.findtext("RefNumber")) for inv in invoices
        if inv.findtext("RefNumber") and inv.findtext("RefNumber").isdigit()
    ]
    next_number = max(used_numbers) + 1 if used_numbers else 10000

    updated_count = 0
    for inv in invoices:
        txnid = inv.findtext("TxnID")
        edit_seq = inv.findtext("EditSequence")
        ref = inv.findtext("RefNumber")

        if not ref and edit_seq:
            try:
                rp.ProcessRequest(
                    ticket,
                    build_invoice_mod(version, txnid, edit_seq, str(next_number))
                )
                log_callback(f"✔ Invoice {txnid} → Assigned #{next_number}")
                next_number += 1
                updated_count += 1
                time.sleep(0.05)
            except Exception as e:
                log_callback(f"❌ Error updating invoice {txnid}: {e}")
    return updated_count

def handle_auto_number_invoices(log, progress):
    try:
        if isinstance(progress, QProgressBar):
            progress.setRange(0, 0)
        log("Running Auto Numbering (Invoices)...")
        rp, ticket = open_connection()
        version = detect_qbxml_version(rp, ticket)

        # 👉 Pehle count dikhado
        invoices = fetch_invoices(rp, ticket, version)
        log(f"📌 Total invoices fetched: {len(invoices)}")

        updated = auto_number_invoices(rp, ticket, version, log)
        close_connection_sdk(rp, ticket)

        log(f"✅ {updated} invoices updated!")
    except Exception as e:
        log(f"❌ Error: {e}")
    finally:
        if isinstance(progress, QProgressBar):
...
def build_bill_mod(version, txnid, edit_seq, ref_number):
    safe_ref = saxutils.escape(ref_number)
    return f"""<?xml version="1.0" encoding="utf-8"?>
<?qbxml version="{version}"?>
<QBXML>
...
def fetch_bills(rp, ticket, version):
    request = build_bill_query(version)
    response = rp.ProcessRequest(ticket, request)
    root = ET.fromstring(response)
    return root.findall(".//BillRet")

def auto_number_bills(rp, ticket, version, log_callback):
    bills = fetch_bills(rp, ticket, version)
    used_numbers = [
        int(bill.findtext("RefNumber")) for bill in bills
        if bill.findtext("RefNumber") and bill.findtext("RefNumber").isdigit()
    ]
    next_number = max(used_numbers) + 1 if used_numbers else 5000

    updated_count = 0
    for bill in bills:
        txnid = bill.findtext("TxnID")
        edit_seq = bill.findtext("EditSequence")
        ref = bill.findtext("RefNumber")

        if not ref and edit_seq:
            try:
                rp.ProcessRequest(
                    ticket,
                    build_bill_mod(version, txnid, edit_seq, str(next_number))
                )
                log_callback(f"✔ Bill {txnid} → Assigned #{next_number}")
                next_number += 1
                updated_count += 1
                time.sleep(0.05)
            except Exception as e:
                log_callback(f"❌ Error updating bill {txnid}: {e}")
    return updated_count


def handle_auto_number_bills(log, progress):
    try:
        if isinstance(progress, QProgressBar):
            progress.setRange(0, 0)
        log("Running Auto Numbering (Bills)...")
        rp, ticket = open_connection()
        version = detect_qbxml_version(rp, ticket)

        # 👉 Pehle count dikhado
        bills = fetch_bills(rp, ticket, version)
        log(f"📌 Total bills fetched: {len(bills)}")

        updated = auto_number_bills(rp, ticket, version, log)
        close_connection_sdk(rp, ticket)

        log(f"✅ {updated} bills updated!")
    except Exception as e:
        log(f"❌ Error: {e}")
    finally:
        if isinstance(progress, QProgressBar):
...
class MainWindow(QMainWindow):
    ...
    def init_numbering_tab(self):
        ...
        btn_num1 = QPushButton("🔢 Invoice Numbering")
        btn_num1.setStyleSheet(self.button_style)
        btn_num1.clicked.connect(lambda: handle_auto_number_invoices(self.log, self.progress))
        row.addWidget(btn_num1)
        
        btn_num2 = QPushButton("🔢 Bill Numbering")
        btn_num2.setStyleSheet(self.button_style)
        btn_num2.clicked.connect(lambda: handle_auto_number_bills(self.log, self.progress))
        row.addWidget(btn_num2)

...
import time
import xml.sax.saxutils as saxutils  # ✅ added for XML escaping
...
def build_account_mod(version, listid, edit_seq, name, acct_type, new_number):
    safe_name = saxutils.escape(name)  # ✅ escape special chars (&, <, >, etc.)
    return f"""<?xml version="1.0" encoding="utf-8"?>
<?qbxml version="{version}"?>
<QBXML>
  <QBXMLMsgsRq onError="stopOnError">
    ...
    for acc in accounts:
        ...
        if not num:
            try:
                ...
                time.sleep(0.05)  # small delay to prevent SDK errors
